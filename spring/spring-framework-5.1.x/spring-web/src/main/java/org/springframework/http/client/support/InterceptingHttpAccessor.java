...
public abstract class InterceptingHttpAccessor extends HttpAccessor {
	...
	public ClientHttpRequestFactory getRequestFactory() {
		// 获取所有的ClientHttpRequestInterceptor
		List<ClientHttpRequestInterceptor> interceptors = getInterceptors();
		// 如果不为空，将InterceptingClientHttpRequestFactory返回
		if (!CollectionUtils.isEmpty(interceptors)) {
			...
			// 如果拦截器为空，直接走SimpleClientHttpRequestFactory（如果配置了RestTemplate的requestFactory那就是自己配置的，默认是基于JDK的URLConnection的SimpleClientHttpRequestFactory），变成一个普通的http调用
