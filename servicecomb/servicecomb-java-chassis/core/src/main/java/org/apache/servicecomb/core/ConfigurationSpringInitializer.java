...
public class ConfigurationSpringInitializer extends PropertySourcesPlaceholderConfigurer implements EnvironmentAware {
  ...
  public void setEnvironment(Environment environment) {
    ...
    // 从Spring的environment同步给servicecomb
    syncFromSpring(environment);
    // 将配置项放到Spring容器中，即：environment
  ...
  private void syncFromSpring(Environment environment) {
    // 获取当前应用名，先从spring.config.name和spring.application.name获取，如果不存在使用environment的hashCode
  ...
  private Map<String, Object> getAllProperties(Environment environment) {
    ...
    // 内部创建了ConcurrentCompositeConfiguration配置对象
    // 将硬件配置，系统配置，yaml等配置项放到ConcurrentCompositeConfiguration对象中
    // 并将配置所有配置中的cse.xxx->servicecomb.xxx
